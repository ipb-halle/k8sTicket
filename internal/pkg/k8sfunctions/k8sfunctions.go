...
func PodToConfig(pod v1.Pod) (proxyfunctions.Config, error) {
	ip := pod.Status.PodIP
	//check if this is a k8sTicket pods
	if value, ok := pod.GetAnnotations()["ipb-halle.de/k8sTicket"]; ok {
		if value == "true" {
			cpath := default_path
			cport := default_port
	...
	log.Println("k8s: ", pod.GetName(), "is not a k8sTicket pod")
	return *new(proxyfunctions.Config), errors.New("No a k8sTicket pod")
}
